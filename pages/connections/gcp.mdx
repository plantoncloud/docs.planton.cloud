...
- **Service Account Key (base64)**  
  ...
  **How to Obtain**:  
  ...
  3. Select or create a service account with the necessary roles for your project (e.g., “Editor” or other role with required permissions).  
  4. Click **Keys** → **Add Key** → **Create new key**.  
  5. Choose **JSON** as the key type. A JSON file will download to your computer.  
  6. Convert this JSON file to a Base64-encoded string (using a command-line tool or an online Base64 encoder).  
  7. Paste that Base64-encoded string into the **Service Account Key (base64)** field.
